...
T = 200; %terminal time
N_time = T*100; %number of time steps
dt = T/N_time;
...
if multistep_flag
    %AB2BDF2 solution
    tic; [both_q_h,~] = AB2BDF2_EM(init_q_h,T,N_time,@middle_k_noise,@HM_stiff,@HM_nonstiff,params_s,params_ns); t = toc;
    both_q_h = [zero_mode zero_mode; both_q_h]; q_h = reshape(both_q_h(:,end),[N,N]); q = ifft2(q_h); %put zero mode back
else
    %ETDRK2 solution
    tic; [q_h,~] = ETDRK2_EM(init_q_h,T,N_time,@middle_k_noise,@HM_stiff,@HM_nonstiff,params_s,params_ns); t = toc;
